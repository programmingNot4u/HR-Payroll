...
const AssetMaintenance = () => {
  ...
  useEffect(() => {
    const loadAssets = async () => {
      try {
        // Load only assets that need maintenance
        const response = await assetService.getMaintenanceDueAssets();
        ...
        console.error("Error loading maintenance due assets:", error);
        setAssets([]);
      }
    };
    loadAssets();
  ...
  const loadMaintenanceHistory = async () => {
    try {
      const maintenanceRecords = await assetService.getAllMaintenanceHistory();

      // Transform the data to match the expected format
      const allMaintenanceHistory = maintenanceRecords.map((record) => ({
        ...record,
        asset: record.asset_name || "Unknown Asset",
        assetId: record.asset_id || record.asset,
        scheduledDate: record.scheduled_date,
        completedDate: record.completed_date,
        maintenanceProvider: record.maintenance_provider,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await assetService.updateAssetStatusForMaintenance(
        selectedAsset,
        "Maintenance"
      );
      ...
      const maintenanceData = {
        ...
        completedDate: null, // Will be filled when maintenance is completed
      ...
      alert(
        'Maintenance request submitted successfully! Asset status updated to "Maintenance".'
      );
    } catch (error) {
  ...
  const handleEditMaintenance = (historyId) => {
    const maintenanceRecord = maintenanceHistory.find(
    ...
    if (maintenanceRecord) {
      ...
      setEditCompletedDate(
        typeof completedDate === "string" ? completedDate : ""
      );
  ...
  const confirmDelete = async () => {
    if (deletingMaintenanceId) {
      ...
      const maintenanceRecord = maintenanceHistory.find(
        (record) => record.id === deletingMaintenanceId
      );
      if (maintenanceRecord) {
        try {
          // Delete from asset service
          await assetService.deleteMaintenanceRecord(
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const completedDate =
        typeof editCompletedDate === "string" ? editCompletedDate : "";
      ...
      await assetService.updateMaintenanceRecord(
        editingMaintenance.assetId,
        editingMaintenance.id,
        {
          maintenance_provider: editMaintenanceProvider,
      ...
      if (completedDate) {
        alert(
          'Maintenance record updated successfully! Asset status updated to "Available".'
        );
      } else {
  ...
  const handleStatusChange = async (historyId, newStatus) => {
    try {
      ...
      if (newStatus === "Complete") {
        ...
        alert(
          `Maintenance status updated to "${newStatus}" successfully! Asset status updated to "${
            newStatus === "Pending" || newStatus === "Ongoing"
              ? "Maintenance"
              : "Available"
          }".`
        );
  ...
  const handleCompletionDateSubmit = async (e) => {
    ...
    try {
      ...
      // Update the maintenance record in asset service
      const maintenanceRecord = maintenanceHistory.find(
        (record) => record.id === completingMaintenanceId
      );
      if (maintenanceRecord) {
        await assetService.updateMaintenanceRecord(
          maintenanceRecord.assetId,
          completingMaintenanceId,
          {
            status: "Complete",
            completed_date: completedDateForStorage,
          }
        );
      ...
      alert(
        'Maintenance status updated to "Complete" successfully! Asset status updated to "Available".'
      );
    } catch (error) {
      console.error("Error updating maintenance status:", error);
      alert(`Error updating maintenance status: ${error.message}`);
  ...
  const handleDatesSubmit = async (e) => {
    ...
    try {
      ...
      // Update the maintenance record in asset service
      const maintenanceRecord = maintenanceHistory.find(
        (record) => record.id === editingDatesId
      );
      if (maintenanceRecord) {
        await assetService.updateMaintenanceRecord(
          maintenanceRecord.assetId,
          editingDatesId,
          {
            scheduled_date: scheduledDateForStorage,
            completed_date: completedDateForStorage,
            status: completedDateForStorage
              ? "Complete"
        ...
        if (completedDateForStorage) {
          ...
          if (
            ...
            maintenanceRecord.status === "Ongoing"
          ) {
            assetStatus = "Maintenance";
          }
        }
              ...
              <div
                ...
                <div className="text-right">
                  <div className="text-sm font-medium text-gray-700">
                    {asset.timeSinceLastMaintenance}
                  </div>
                  <div className="text-xs text-gray-500">
                    {asset.lastMaintenanceDate
                      ? `Last: ${formatDateToDDMMYYYY(
                          asset.lastMaintenanceDate
                        )}`
                  ...
                  <tr key={history.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-gray-900">
                          {formatDateToDDMMYYYY(history.scheduledDate)}
                        </span>
                        <button
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-gray-900">
                          {history.completedDate
                            ? formatDateToDDMMYYYY(history.completedDate)
                            : "-"}
                        </span>
                        <button
                          onClick={() => handleEditDates(history.id)}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <select
                        ...
                        onChange={(e) =>
                          handleStatusChange(history.id, e.target.value)
                        }
            ...
            <form onSubmit={handleDatesSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setEditCompletedDateNew(e.target.value)}
                  className="w-full p-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-orange-500"
                />
