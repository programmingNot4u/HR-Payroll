...
const compressImage = (file, maxSizeKB = 20, quality = 0.8) => {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      // Calculate new dimensions to maintain aspect ratio - very small for minimal file size
      const maxWidth = 150; // Very small dimensions for minimal file size
      const maxHeight = 150;
...
export default function AddEmployee() {
  ...
  useEffect(() => {
    const loadOrganizationalData = async () => {
      try {
        ...
        setAllMachines(organizationalDataService.getMachines());
  ...
  const [formData, setFormData] = useState(() => {
    ...
    return {
      ...
      // Organizational data IDs for proper backend integration
      departmentId: "",
      designationId: "",
      salaryGradeId: "",
      ...
      role: "employee", // User role: employee, hr_staff, hr_manager, department_head
      email: "", // Email for HR users
      offDay: "",
      unit: "",
      designation: "",
      department: "",
  ...
  const updateFormData = useCallback(
    (field, value) => {
      ...
      setFormData((prev) => {
        ...
        if (field === "employeeId") {
          console.log("newData.employeeId:", newData.employeeId);
        }
        return newData;
      });
  ...
  const handleDateOfJoiningChange = useCallback(
    (e) => {
  ...
  // Function to show credentials modal
  const showCredentialsModalFunction = useCallback(
    (newEmployee, employeeType) => {
      const isHR =
        newEmployee.role === "hr_manager" ||
        newEmployee.role === "hr_staff" ||
        newEmployee.role === "department_head";

      console.log("=== CREDENTIALS MODAL DEBUG ===");
      console.log("Employee role:", newEmployee.role);
  ...
  const handleDateOfIssueChange = useCallback(
    (e) => {
  ...
  const calculateWorkerSalary = useCallback(
    (grossSalary) => {
      ...
      const selectedGrade = salaryGrades.find(
        (grade) => grade.name === formData.salaryGrade
      );
      ...
      if (selectedGrade && selectedGrade.type === "Worker") {
        setFormData((prev) => ({
          ...
          salaryComponents: {
            basicSalary: {
              ...prev.salaryComponents.basicSalary,
              amount: selectedGrade.basicSalary,
              custom: false,
            },
            houseRent: {
              ...prev.salaryComponents.houseRent,
              amount: selectedGrade.houseRent,
              custom: false,
            },
            medicalAllowance: {
              ...prev.salaryComponents.medicalAllowance,
              amount: selectedGrade.medicalAllowance,
              custom: false,
            },
            foodAllowance: {
              ...prev.salaryComponents.foodAllowance,
              amount: selectedGrade.foodAllowance || 0,
              custom: false,
            },
            conveyance: {
              ...prev.salaryComponents.conveyance,
              amount: selectedGrade.conveyance,
              custom: false,
            },
            mobileBill: {
              ...prev.salaryComponents.mobileBill,
              amount: 0,
              custom: false,
            }, // Mobile bill not for workers
          },
        }));
      }
    },
  ...
  const calculateStaffSalary = useCallback(
    (grossSalary) => {
      if (!grossSalary || grossSalary <= 0) return;
  ...
  ]); // Dependencies for salary calculation
  ...
  const validateBulkDocuments = () => {
    ...
    bulkDocuments.forEach((emp, index) => {
      ...
      // Employee ID validation - optional field, but if provided, should not be empty
      if (emp.employeeId?.trim()) {
        // Allow any non-empty Employee ID format for existing company migrations
        if (emp.employeeId.trim().length < 2) {
          empErrors.push("Employee ID must be at least 2 characters long");
        }
      }
      if (!emp.nameEnglish) {
      ...
      if (emp.employeeType === "Worker") {
        ...
        if (!emp.child1Name && !emp.child2Name && !emp.child3Name) {
          empErrors.push(
            "At least one child information is required for Workers"
          );
        }
        // At least one nominee should be provided for workers
        if (!emp.nominee1Name && !emp.nominee2Name) {
          empErrors.push(
            "At least one nominee information is required for Workers"
          );
        }
      }
    ...
    try {
      ...
      const templateData = [
        ...
        [
          ...
          "Efficiency 1 Process",
          "Efficiency 1 Delivery",
          "Efficiency 2 Process",
          "Efficiency 2 Delivery",
        ],
        // Sample data row
        [
          "Worker",
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.employeeId?.trim()) {
      // Allow any non-empty Employee ID format for existing company migrations
      if (formData.employeeId.trim().length < 2) {
    ...
    if (!formData.salaryGradeId) {
      errors.push("Salary Grade is required");
    }
    ...
    if (typeof formData.presentAddress === "string") {
      ...
      typeof formData.presentAddress === "object" &&
      formData.presentAddress !== null
    ) {
      ...
      if (
        ...
        errors.push(
          "Present Address is required (at least Village and District must be filled)"
        );
      }
    } else {
      errors.push("Present Address is required");
    ...
    if (typeof formData.permanentAddress === "string") {
      ...
      typeof formData.permanentAddress === "object" &&
      ...
      if (
        ...
        errors.push(
          "Permanent Address is required (at least Village and District must be filled)"
        );
      }
    } else {
      errors.push("Permanent Address is required");
    ...
    if (employeeType === "Worker") {
      ...
      const hasValidNominee = formData.nominee.some(
        (nominee) => nominee.name?.trim() && nominee.mobile?.trim()
      );
    ...
    const confirmMessage = `Are you sure you want to add this ${employeeType.toLowerCase()}?\n\nName: ${
      ...
      formData.salaryGrade || "Not selected"
    ...
    if (window.confirm(confirmMessage)) {
      try {
        ...
        // Compress picture file if present
        let compressedPicture = null;
        if (formData.picture && formData.picture instanceof File) {
          try {
            console.log(
              "Original picture size:",
              (formData.picture.size / 1024).toFixed(2),
              "KB"
            );
            showWarning("Compressing image, please wait...");

            // Compress image to 5-10KB with extremely small dimensions
            ...
            console.log(
              ...
              (compressedPicture.size / 1024).toFixed(2),
              "KB"
            );
            console.log("Compressed picture type:", typeof compressedPicture);
            console.log("Is File instance:", compressedPicture instanceof File);
            console.log("File name:", compressedPicture.name);
            console.log("File type:", compressedPicture.type);
            showSuccess(
              `Image compressed to ${(compressedPicture.size / 1024).toFixed(
                1
              )}KB`
            );
          } catch (error) {
            console.error("Error compressing picture file:", error);
            showError("Error processing picture file. Please try again.");
            return;
          }
        }
        // Debug picture field
        console.log("Picture field type:", typeof formData.picture);
        console.log("Picture field value:", formData.picture);
        console.log("Is File instance:", formData.picture instanceof File);
        console.log("Compressed picture type:", typeof compressedPicture);
        console.log("Compressed picture value:", compressedPicture);
        console.log(
          "Compressed picture is File:",
          compressedPicture instanceof File
        );

        // Debug gender field
        console.log("Gender field value:", formData.gender);
        console.log("Gender field type:", typeof formData.gender);
        console.log("Gender field === 'male':", formData.gender === "male");
        console.log("Gender field === 'Male':", formData.gender === "Male");
        console.log(
          "Gender field after toLowerCase:",
          formData.gender?.toLowerCase()
        );

        // Prepare employee data for API submission
        const employeeData = {
          // User fields (required for user creation)
          first_name: formData.nameEnglish?.split(" ")[0] || "",
          last_name:
            formData.nameEnglish?.split(" ").slice(1).join(" ") ||
            formData.nameEnglish ||
            "",
          phone: formData.mobileNumber,
          role: formData.role,

          // Employee ID (manual or auto-generated)
          employee_id: formData.employeeId?.trim() || null,

          // Email (manual or auto-generated)
          email: formData.email?.trim() || null,

          // Organizational fields
          department: formData.departmentId,
          designation: formData.designationId,
          salary_grade: formData.salaryGradeId,
          level_of_work: formData.levelOfWork?.toLowerCase(),
          status: "active",

          // Personal Information
          picture: compressedPicture || null,
          ...
          // Process Expertise (for Workers)
          process_expertise: formData.processExpertise || [],
          process_efficiency: formData.processEfficiency || [],
          ...
          // Salary Components
          salary_components: formData.salaryComponents || {},
        };

        // Save employee using the service
        console.log("About to send employee data:", employeeData);
        console.log("=== DEBUG EMPLOYEE ID ===");
        console.log("formData.employeeId:", formData.employeeId);
        console.log("formData.employeeId type:", typeof formData.employeeId);
        console.log("formData.employeeId length:", formData.employeeId?.length);
        console.log("employeeData.employee_id:", employeeData.employee_id);
        console.log(
          "employeeData.employee_id type:",
          typeof employeeData.employee_id
        );
        console.log("=== END DEBUG ===");
        console.log("Picture field in employeeData:", employeeData.picture);
        console.log(
          "Picture field type in employeeData:",
          typeof employeeData.picture
        );
        console.log(
          "Is picture a File instance:",
          employeeData.picture instanceof File
        );

        // Debug JSON fields
        console.log(
          "present_address type:",
          typeof employeeData.present_address
        );
        console.log("present_address value:", employeeData.present_address);
        console.log(
          "permanent_address type:",
          typeof employeeData.permanent_address
        );
        console.log("permanent_address value:", employeeData.permanent_address);
        console.log(
          "process_efficiency type:",
          typeof employeeData.process_efficiency
        );
        console.log(
          "process_efficiency value:",
          employeeData.process_efficiency
        );
        console.log("nominee type:", typeof employeeData.nominee);
        console.log("nominee value:", employeeData.nominee);
        console.log(
          "emergency_contact type:",
          typeof employeeData.emergency_contact
        );
        console.log("emergency_contact value:", employeeData.emergency_contact);
        ...
        console.log(
          "Picture:",
          formData.picture ? formData.picture.name : "No picture uploaded"
        );

        console.log("=== EMPLOYEE SUCCESSFULLY ADDED ===");
        console.log("New Employee ID:", newEmployee.employee_id);
        console.log("Employee Name:", newEmployee.name_english);

        // Show success message
        showSuccess(`${employeeType} added successfully!`);

        // Show credentials modal for all employees
        showCredentialsModalFunction(newEmployee, employeeType);

        // Dispatch event to refresh Employee Dashboard
        window.dispatchEvent(
          new CustomEvent("employeeAdded", {
            detail: {
              employee: newEmployee,
              employeeType: employeeType,
              levelOfWork: formData.levelOfWork,
        ...
        const employeeId = newEmployee.id || newEmployee.employee_id;
        localStorage.setItem("selectedEmployeeId", employeeId);
        console.log("Setting selectedEmployeeId to:", employeeId);

        // Note: Navigation will happen when user clicks "Finish" in credentials modal
      } catch (error) {
      ...
      setFormData({
        ...
        // Emergency & Nominee
        nominee: [
          { name: "", mobile: "", nidBirthCertificate: "" },
          { name: "", mobile: "", nidBirthCertificate: "" },
        ],
      ...
      <div className="rounded border border-gray-200 bg-white p-6">
        ...
        <div className="flex gap-4">
          ...
          <button
            ...
            style={
              employeeType === "Staff"
                ? { backgroundColor: "rgb(255,185,125)" }
                : {}
            }
            onMouseEnter={(e) => {
          ...
          <div className="mt-6 space-y-6">
              ...
              <div className="flex items-start">
                ...
                <div className="ml-3">
                  <h3 className="text-sm font-medium text-blue-800">
                    Picture Upload Information
                  </h3>
                  <div className="mt-2 text-sm text-blue-700">
                    <p>
                      • The Excel template includes a "Picture Path" column for
                      employee photos
                    </p>
                    <p>
                      • Enter the file path or URL of the employee's picture in
                      this column
                    </p>
                    <p>
                      • Example:{" "}
                ...
                <div className="space-y-2">
                  {uploadErrors.map((error, index) => (
                    <div key={index} className="text-sm text-red-700">
                      ...
                      {error.employeeId && (
                        <p>
                          Row {error.row}: Employee ID "{error.employeeId}" -{" "}
                          {error.errors.join(", ")}
                        </p>
                      )}
                    </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              ...
              <ul className="text-sm text-blue-700 space-y-1">
                <li>
                  • Download the template and fill in the employee information
                </li>
                <li>
                  • Required fields: Name (English), Name (Bangla), Department,
                  Designation, etc.
                </li>
                <li>
                  • Employee IDs: Leave empty for auto-generation, or enter
                  custom ID (any format accepted)
                </li>
                <li>• Supported file formats: .xlsx, .xls</li>
                <li>
                  • All form fields are included in the template for complete
                  employee data
                </li>
              </ul>
            </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (
                    !["Hon's", "Master's", "BSc", "MSc", "PhD"].includes(
                      selectedLevel
                    )
                  ) {
        ...
        {employeeType === "Worker" && (
          <div className="rounded border border-gray-200 bg-white p-6">
            ...
            <p className="text-sm text-gray-600 mb-4">
              You can add children information if available. This section is
              completely optional.
            </p>
            <div className="space-y-4">
              {formData.children.map((child, index) => (
                <div
                  key={index}
                  className="relative grid grid-cols-1 md:grid-cols-4 gap-4 p-4 border border-gray-200 rounded">
                  {/* Remove button */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  updateNestedField("presentAddress", "village", e.target.value)
                }
                className="w-full h-10 rounded border border-gray-300 px-3 focus:outline-none focus:ring-2"
              />
            </div>
        ...
        <div className="rounded border border-gray-200 bg-white p-6">
          ...
          <div className="mb-4">
            ...
            <select
              ...
              onChange={(e) =>
                updateFormData("hasWorkExperience", e.target.value)
              }
              className="w-full h-10 rounded border border-gray-300 px-3 focus:outline-none focus:ring-2">
          ...
          {formData.hasWorkExperience === "Yes" && (
            <div className="space-y-4">
              {formData.workExperience.map((exp, index) => (
                <div
                  key={index}
                  className="grid grid-cols-1 md:grid-cols-5 gap-4 p-4 border border-gray-200 rounded">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Company Name
                    </label>
          ...
          <div className="rounded border border-gray-200 bg-white p-6">
            ...
            <div className="space-y-4">
              {formData.processExpertise.map((expertise, index) => (
                <div
                  key={index}
                  className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 border border-gray-200 rounded">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Operation
                    </label>
                    <select
                      ...
                      onChange={(e) =>
                        updateOperationAndMachines(index, e.target.value)
                      }
                      className="w-full h-10 rounded border border-gray-300 px-3 focus:outline-none focus:ring-2">
              ...
              <div
                ...
                <div>
                  ...
                  <input
                    ...
                    value={nominee.name}
                    onChange={(e) =>
                      updateArrayField("nominee", index, "name", e.target.value)
                    }
                    className="w-full h-10 rounded border border-gray-300 px-3 focus:outline-none focus:ring-2"
                    placeholder="Hassan"
                  />
        ...
        <div className="rounded border border-gray-200 bg-white p-6">
          <h2
            ...
            Department Only For Administration
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
              ...
              <input
                ...
                placeholder="Enter Employee ID (any format) or leave empty for auto-generation"
                className="w-full h-10 rounded border border-gray-300 px-3 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent"
              />
              <p className="text-xs text-gray-500 mt-1">
                Leave empty to auto-generate, or enter a custom Employee ID
              </p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
            ...
            <div>
              ...
              <input
                ...
                className="w-full h-10 rounded border border-gray-300 px-3 focus:outline-none focus:ring-2"
                placeholder="Enter email address for system login"
                required
              />
              <p className="text-xs text-gray-500 mt-1">
            ...
            <div>
              ...
              />
              <p className="text-xs text-gray-500 mt-1">
                Level of Work is automatically set to match the selected
                employee type
              </p>
            </div>
            <div>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  updateFormData("department", selectedDept?.name || "");
                }}
                className="w-full h-10 rounded border border-gray-300 px-3 focus:outline-none focus:ring-2">
                <option value="">Select Department</option>
                {departments.map((department) => (
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  updateFormData("designationId", e.target.value);
                  updateFormData("designation", selectedDesig?.name || "");
                }}
                className="w-full h-10 rounded border border-gray-300 px-3 focus:outline-none focus:ring-2">
                <option value="">Select Designation</option>
                {designations.map((designation) => (
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  updateFormData("salaryGradeId", e.target.value);
                  updateFormData("salaryGrade", selectedGrade?.name || "");
                  if (selectedGrade) {
                    updateFormData("grossSalary", selectedGrade.gross_salary);
                  }
                }}
                className="w-full h-10 rounded border border-gray-300 px-3 focus:outline-none focus:ring-2">
                ...
                {(employeeType === "Worker"
                  ...
                  <option key={grade.id} value={grade.id}>
                    {grade.name} - ৳{grade.gross_salary.toLocaleString()}
                  </option>
                ))}
              </select>
            </div>
                ...
                <tr className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-center">
                    <input
                      type="number"
                      value={
                        formData.salaryComponents?.basicSalary?.amount || 0
                      }
                      onChange={(e) =>
                        updateSalaryComponent("basicSalary", e.target.value)
                      }
                ...
                <tr className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-center">
                    <input
                      ...
                      onChange={(e) =>
                        updateSalaryComponent("houseRent", e.target.value)
                      }
                      className="w-24 h-8 text-center border border-gray-300 rounded focus:outline-none focus:ring-2"
                    />
                  </td>
                ...
                <tr className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-center">
                    <input
                      type="number"
                      value={
                        formData.salaryComponents?.medicalAllowance?.amount || 0
                      }
                      onChange={(e) =>
                  ...
                  <tr className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-center">
                      <input
                        type="number"
                        value={
                          formData.salaryComponents?.foodAllowance?.amount || 0
                        }
                        onChange={(e) =>
                          updateSalaryComponent("foodAllowance", e.target.value)
                        }
                        className="w-24 h-8 text-center border border-gray-300 rounded focus:outline-none focus:ring-2"
                      />
                    </td>
                ...
                <tr className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-center">
                    <input
                      ...
                      onChange={(e) =>
                        updateSalaryComponent("conveyance", e.target.value)
                      }
                      className="w-24 h-8 text-center border border-gray-300 rounded focus:outline-none focus:ring-2"
                    />
                  </td>
                  ...
                  <tr className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-center">
                      <input
                        type="number"
                        value={
                          formData.salaryComponents?.mobileBill?.amount || 0
                        }
                        onChange={(e) =>
                          updateSalaryComponent("mobileBill", e.target.value)
                        }
                        className="w-24 h-8 text-center border border-gray-300 rounded focus:outline-none focus:ring-2"
                      />
                    </td>
        ...
        <div className="flex justify-between items-center">
          ...
          <button
            ...
            onClick={() => {
              if (!hasSeenPreview) {
                showWarning(
                  'Please review the form by clicking "See Preview" before submitting.'
                );
                return;
              }
              // If preview has been shown, allow direct submission
              console.log(
                "Submit button clicked, hasSeenPreview:",
                hasSeenPreview
              );
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div
              ...
              style={{
                background:
                  "linear-gradient(to right, rgb(255,250,245), rgb(255,240,220))",
              }}>
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-lg text-gray-600">
                    Company Name: RP Creations & Apparels Limited
                  </p>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="border-2 border-gray-300 rounded-lg p-4">
                <h3 className="text-xl font-bold mb-4 text-blue-800 border-b-2 border-blue-800 pb-2">
                  Personal Information
                </h3>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div className="flex">
                    ...
                    <span className="border-b border-gray-400 flex-1 px-2">
                      {formData.nidNumber || "_________________"}
                    </span>
                  </div>
                  <div className="flex">
                    <span className="font-bold w-32">Blood Group:</span>
                    <span className="border-b border-gray-400 flex-1 px-2">
                      {formData.bloodGroup || "_________________"}
                    </span>
                  </div>
                  <div className="flex">
                    <span className="font-bold w-32">Religion:</span>
                    <span className="border-b border-gray-400 flex-1 px-2">
                      {formData.religion || "_________________"}
                    </span>
                  </div>
                  <div className="flex">
                    <span className="font-bold w-32">Marital Status:</span>
                    <span className="border-b border-gray-400 flex-1 px-2">
                      {formData.maritalStatus || "_________________"}
                    </span>
                  </div>
                  <div className="flex">
                    <span className="font-bold w-32">Gender:</span>
                    <span className="border-b border-gray-400 flex-1 px-2">
                      {formData.gender || "_________________"}
                    </span>
                  </div>
                </div>
              </div>
              ...
              {/* Address Information */}
              <div className="border-2 border-gray-300 rounded-lg p-4">
                <h3 className="text-xl font-bold mb-4 text-green-800 border-b-2 border-green-800 pb-2">
                  Address Information
                </h3>
                  ...
                  <div>
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex">
                        ...
                        <span className="border-b border-gray-400 flex-1 px-2">
                          {formData.presentAddress.houseOwnerName ||
                            "_________________"}
                        </span>
                      </div>
                      <div className="flex">
                      ...
                      <div className="flex">
                        ...
                        <span className="border-b border-gray-400 flex-1 px-2">
                          {formData.presentAddress.postOffice ||
                            "_________________"}
                        </span>
                      </div>
                      <div className="flex">
                        ...
                        <span className="border-b border-gray-400 flex-1 px-2">
                          {formData.presentAddress.upazilla ||
                            "_________________"}
                        </span>
                      </div>
                      <div className="flex">
                  ...
                  <div>
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex">
                        <span className="font-bold w-24">House:</span>
                        <span className="border-b border-gray-400 flex-1 px-2">
                          {formData.permanentAddress.houseOwnerName ||
                            "_________________"}
                        </span>
                      </div>
                      <div className="flex">
                      ...
                      <div className="flex">
                        <span className="font-bold w-24">Post Office:</span>
                        <span className="border-b border-gray-400 flex-1 px-2">
                          {formData.permanentAddress.postOffice ||
                            "_________________"}
                        </span>
                      </div>
                      <div className="flex">
                        <span className="font-bold w-24">Upazilla:</span>
                        <span className="border-b border-gray-400 flex-1 px-2">
                          {formData.permanentAddress.upazilla ||
                            "_________________"}
                        </span>
                      </div>
                      <div className="flex">
              ...
              <div className="border-2 border-gray-300 rounded-lg p-4">
                <h3
                  ...
                  style={{
                    color: "rgb(200,100,50)",
                    borderBottom: "2px solid rgb(255,200,150)",
                  }}>
                  Employment Information
                </h3>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className="flex">
                    ...
                    <span className="border-b border-gray-400 flex-1 px-2">
                      {formData.employeeId || "_________________"}
                    </span>
                  </div>
                  <div className="flex">
                    <span className="font-bold w-32">Level of Work:</span>
                    <span className="border-b border-gray-400 flex-1 px-2">
                      {formData.levelOfWork || "_________________"}
                    </span>
                  </div>
                  <div className="flex">
                    <span className="font-bold w-32">Designation:</span>
                    <span className="border-b border-gray-400 flex-1 px-2">
                      {formData.designation || "_________________"}
                    </span>
                  </div>
                  <div className="flex">
                    <span className="font-bold w-32">Department:</span>
                    <span className="border-b border-gray-400 flex-1 px-2">
                      {formData.department || "_________________"}
                    </span>
                  </div>
                  {employeeType === "Worker" && (
                    <>
                      <div className="flex">
                        ...
                        <span className="border-b border-gray-400 flex-1 px-2">
                          {formData.unit || "_________________"}
                        </span>
                      </div>
                      <div className="flex">
                        <span className="font-bold w-32">Line:</span>
                        <span className="border-b border-gray-400 flex-1 px-2">
                          {formData.line || "_________________"}
                        </span>
                      </div>
                      <div className="flex">
                        <span className="font-bold w-32">Supervisor:</span>
                        <span className="border-b border-gray-400 flex-1 px-2">
                          {formData.supervisorName || "_________________"}
                        </span>
                      </div>
                    </>
                  )}
                  ...
                  <div className="flex">
                    ...
                    <span className="border-b border-gray-400 flex-1 px-2">
                      {formData.salaryGrade || "_________________"}
                    </span>
                  </div>
                  <div className="flex">
                    <span className="font-bold w-32">Gross Salary:</span>
                    <span className="border-b border-gray-400 flex-1 px-2">
                      ৳{formData.grossSalary || "_________________"}
                    </span>
                  </div>
                </div>
              </div>
              ...
              {/* Salary Components */}
              <div className="border-2 border-gray-300 rounded-lg p-4">
                <h3 className="text-xl font-bold mb-4 text-purple-800 border-b-2 border-purple-800 pb-2">
                  Salary Breakdown
                </h3>
                <div className="space-y-2 text-sm">
                  {Object.entries(formData.salaryComponents).map(
                    ([key, component]) =>
                      component.enabled && (
                        <div
                          key={key}
              ...
              {formData.workExperience.some((exp) => exp.company) && (
                <div className="border-2 border-gray-300 rounded-lg p-4">
                  <h3 className="text-xl font-bold mb-4 text-cyan-800 border-b-2 border-cyan-800 pb-2">
                    Working Experience
                  </h3>
                  <div className="space-y-3 text-sm">
                    {formData.workExperience
                      ...
                      .map((exp, index) => (
                        <div
                          key={index}
                          className="border-l-4 border-cyan-400 pl-3">
              ...
              {employeeType === "Worker" && (
                <>
                  {formData.children.some((child) => child.name) && (
                    <div className="border-2 border-gray-300 rounded-lg p-4">
                      <h3 className="text-xl font-bold mb-4 text-indigo-800 border-b-2 border-indigo-800 pb-2">
                        Children Information
                      </h3>
                      <div className="space-y-3 text-sm">
                        {formData.children
                  ...
                  {formData.processExpertise.some(
                    (expertise) => expertise.operation
                  ) && (
                    <div className="border-2 border-gray-300 rounded-lg p-4">
                      <h3 className="text-xl font-bold mb-4 text-yellow-800 border-b-2 border-yellow-800 pb-2">
                        Process Expertise
                      </h3>
                      <div className="space-y-3 text-sm">
                        {formData.processExpertise
                          .filter((expertise) => expertise.operation)
                          .map((expertise, index) => (
                            <div
                              key={index}
                  ...
                  {formData.processEfficiency.some(
                    (efficiency) => efficiency.itemDescription
                  ) && (
                    <div className="border-2 border-gray-300 rounded-lg p-4">
                      <h3 className="text-xl font-bold mb-4 text-teal-800 border-b-2 border-teal-800 pb-2">
                        Process Efficiency
                      </h3>
                      <div className="space-y-3 text-sm">
                        {formData.processEfficiency
                          .filter((efficiency) => efficiency.itemDescription)
                          .map((efficiency, index) => (
                            <div
                              key={index}
                              className="border-l-4 border-teal-400 pl-3">
                              <div className="grid grid-cols-3 gap-4">
                                <div className="flex">
                                  <span className="font-bold w-20">
                                    Item Description:
                  ...
                  {formData.nominee.some((nominee) => nominee.name) && (
                    <div className="border-2 border-gray-300 rounded-lg p-4">
                      <h3 className="text-xl font-bold mb-4 text-red-800 border-b-2 border-red-800 pb-2">
                        Nominee
                      </h3>
                      <div className="space-y-3 text-sm">
                        {formData.nominee
              ...
              <div className="border-2 border-gray-300 rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className="flex">
                    ...
                    <span className="border-b border-gray-400 flex-1 px-2">
                      {formData.emergencyContact.name || "_________________"}
                    </span>
                  </div>
                  <div className="flex">
                    <span className="font-bold w-32">Mobile:</span>
                    <span className="border-b border-gray-400 flex-1 px-2">
                      {formData.emergencyContact.mobile || "_________________"}
                    </span>
                  </div>
                  <div className="flex">
                ...
                <div className="flex flex-wrap gap-3">
                  <button
                    onClick={() => {
                      ...
                      const cleanActionButtons = cleanContent.querySelector(
                  ...
                  <button
                    onClick={() => {
                      ...
                      const shareData = {
                        ...
                        text: `Employee: ${
                          formData.nameEnglish || "Draft"
                        }\nID: ${
                          formData.employeeId || "TBD"
                        }\nType: ${employeeType}`,
                    ...
                    onMouseEnter={(e) =>
                      (e.target.style.backgroundColor = "rgb(255,155,75)")
                    }
                    onMouseLeave={(e) =>
                      (e.target.style.backgroundColor = "rgb(255,170,100)")
                    }>
          ...
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <div className="text-center mb-6">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">
                {newEmployeeCredentials.employeeType} Created Successfully!
              </h3>
              <p className="text-sm text-gray-500">
                Please share these login credentials with the new{" "}
                {newEmployeeCredentials.isHR ? "HR Manager" : "employee"}
              </p>
              ...
              <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                ...
                {newEmployeeCredentials.isHR ? (
                  <div className="text-sm text-blue-800">
                    <p className="mb-2">
                      <strong>This HR Manager has access to:</strong>
                    </p>
                    <ul className="list-disc list-inside space-y-1">
                      <li>
                        <strong>Employee Portal:</strong> Use Employee ID +
                        Password
                      </li>
                      <li>
                        <strong>HR Admin Portal:</strong> Use Email/Phone +
                        Password
                      </li>
                    </ul>
                  </div>
                  ...
                  <div className="text-sm text-blue-800">
                    <p className="mb-2">
                      <strong>This Employee has access to:</strong>
                    </p>
                    <ul className="list-disc list-inside space-y-1">
                      <li>
                        <strong>Employee Portal Only:</strong> Use Employee ID +
                        Password
                      </li>
                    </ul>
                  </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-lg font-mono bg-white px-3 py-2 rounded border flex-1 mr-2">
                    {newEmployeeCredentials.employee.employee_id}
                  </span>
                  <button
                    onClick={() =>
                      copyToClipboard(
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm font-mono bg-white px-3 py-2 rounded border flex-1 mr-2 break-all">
                    {newEmployeeCredentials.employee.generated_email}
                  </span>
                  <button
                    onClick={() =>
                      copyToClipboard(
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm font-mono bg-white px-3 py-2 rounded border flex-1 mr-2">
                    {newEmployeeCredentials.employee.generated_password ||
                      "Not generated"}
                  </span>
                  <button
                    onClick={() =>
              ...
              <div className="pt-4">
                <button
                  onClick={() => {
                    const allCredentials = `Employee ID: ${
                      newEmployeeCredentials.employee.employee_id
                    }\nEmail: ${
                      newEmployeeCredentials.employee.generated_email
                    }\nPassword: ${
                    ...
                    copyToClipboard(allCredentials);
                  }}
            ...
            <div className="mt-6 flex gap-3">
              <button
                onClick={() => {
                  ...
                  // Navigate to Employee Details after closing modal
                  const event = new CustomEvent("navigateTo", {
                    detail: "Employee Details",
                  });
                  window.dispatchEvent(event);
                }}
                className="flex-1 px-4 py-2 bg-orange-500 text-white rounded hover:bg-orange-600 transition-colors font-semibold">
                Finish
              </button>
            </div>
          </div>
