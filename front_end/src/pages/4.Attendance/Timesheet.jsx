...
import { timesheetAPI } from "../../services/attendanceService";
...
export default function Timesheet() {
  ...
  const loadTimesheetData = async (month, year) => {
    try {
      setLoading(true);
      setError(null);
      const data = await timesheetAPI.getTimesheetData(month, year);
      ...
      setAttendanceData(
        generateEmployeeAttendanceData(parseInt(month), parseInt(year))
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    loadTimesheetData(filters.month, filters.year);
  ...
  const filteredAttendance = useMemo(() => {
    return attendanceData.filter((employee) => {
      ...
      const matchesLevelOfWork =
        ...
        employee.levelOfWork === filters.levelOfWork;
  ...
  const summaryStats = useMemo(() => {
    ...
    filteredAttendance.forEach((employee) => {
      employee.days.forEach((day) => {
        ...
        else if (day.status === "Present-Considered")
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500">
          Complete attendance report for all employees for{" "}
          {months.find((m) => m.value === filters.month)?.label} {filters.year}
        </p>
      </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div className="flex items-center">
              <span className="inline-block w-4 h-4 bg-green-100 text-green-800 rounded-full text-xs font-semibold text-center mr-2">
                P
              </span>
              <span className="text-gray-600">Present-OnTime</span>
            </div>
            <div className="flex items-center">
              <span className="inline-block w-4 h-4 bg-yellow-100 text-yellow-800 rounded-full text-xs font-semibold text-center mr-2">
                C
              </span>
              <span className="text-gray-600">Present-Considered</span>
            </div>
            <div className="flex items-center">
              <span className="inline-block w-4 h-4 bg-orange-100 text-orange-800 rounded-full text-xs font-semibold text-center mr-2">
                L
              </span>
              <span className="text-gray-600">Present-Late</span>
            </div>
            <div className="flex items-center">
              <span className="inline-block w-4 h-4 bg-blue-100 text-blue-800 rounded-full text-xs font-semibold text-center mr-2">
                E
              </span>
              <span className="text-gray-600">Leave-Earn</span>
            </div>
            <div className="flex items-center">
              <span className="inline-block w-4 h-4 bg-purple-100 text-purple-800 rounded-full text-xs font-semibold text-center mr-2">
                M
              </span>
              <span className="text-gray-600">Leave-Maternity</span>
            </div>
            <div className="flex items-center">
              <span className="inline-block w-4 h-4 bg-indigo-100 text-indigo-800 rounded-full text-xs font-semibold text-center mr-2">
                C
              </span>
              <span className="text-gray-600">Leave-Casual</span>
            </div>
            <div className="flex items-center">
              <span className="inline-block w-4 h-4 bg-pink-100 text-pink-800 rounded-full text-xs font-semibold text-center mr-2">
                S
              </span>
              <span className="text-gray-600">Leave-Sick</span>
            </div>
            <div className="flex items-center">
              <span className="inline-block w-4 h-4 bg-gray-100 text-gray-800 rounded-full text-xs font-semibold text-center mr-2">
                W
              </span>
              <span className="text-gray-600">Leave-WithOutPay</span>
            </div>
            <div className="flex items-center">
              <span className="inline-block w-4 h-4 bg-red-100 text-red-800 rounded-full text-xs font-semibold text-center mr-2">
                A
              </span>
              <span className="text-gray-600">Absent</span>
            </div>
            <div className="flex items-center">
              <span className="inline-block w-6 h-4 bg-slate-100 text-slate-600 rounded-full text-xs font-semibold text-center mr-2">
                WE
              </span>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h3 className="text-lg font-medium text-gray-900">
            ...
            {filters.year} Attendance Report
          </h3>
        </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredAttendance.map((employee) => {
                ...
                const legendCounts = {
                  ...
                  weekend: employee.days.filter(
                    (day) => day.status === "Weekend"
                  ).length,
                  holidays: employee.days.filter(
                    (day) => day.status === "Holiday"
                  ).length, // Will be 0 for now as we don't have holiday logic
                  presentDay: employee.days.filter(
                    (day) =>
                      day.status === "Present-OnTime" ||
                      day.status === "Present-Considered" ||
                      day.status === "Present-Late"
                  ).length,
                  presentOnTime: employee.days.filter(
                    (day) => day.status === "Present-OnTime"
                  ).length,
                  presentConsidered: employee.days.filter(
                    (day) => day.status === "Present-Considered"
                  ).length,
                  presentLate: employee.days.filter(
                    (day) => day.status === "Present-Late"
                  ).length,
                  leaveEarn: employee.days.filter(
                    (day) => day.status === "Leave-Earn"
                  ).length,
                  leaveMaternity: employee.days.filter(
                    (day) => day.status === "Leave-Maternity"
                  ).length,
                  leaveCasual: employee.days.filter(
                    (day) => day.status === "Leave-Casual"
                  ).length,
                  leaveSick: employee.days.filter(
                    (day) => day.status === "Leave-Sick"
                  ).length,
                  leaveWithoutPay: employee.days.filter(
                    (day) => day.status === "Leave-WithOutPay"
                  ).length,
                  absent: employee.days.filter((day) => day.status === "Absent")
                    .length,
                };
                    ...
                    {monthDates.map((dateInfo) => {
                      ...
                      return (
                        <td
                          ...
                          <span
                            className={`inline-flex items-center justify-center w-6 h-6 text-xs font-semibold rounded-full ${getStatusColor(
                              status
                            )}`}>
