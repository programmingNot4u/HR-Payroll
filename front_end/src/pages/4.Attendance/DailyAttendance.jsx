...
// Sample attendance data (fallback if API fails)
...
export default function DailyAttendance() {
  ...
  const loadAttendanceData = async (date) => {
    try {
      setLoading(true);
      setError(null);
      const data = await dailyAttendanceAPI.getDailySummary(date);
  ...
  useEffect(() => {
    const websocket = new AttendanceWebSocket();
    websocket.connect();
    setWs(websocket);

    // Listen for real-time updates
    const handleScanReceived = (event) => {
      console.log("New scan received:", event.detail);
      // Refresh data when new scan is received
    ...
    window.addEventListener("attendanceScanReceived", handleScanReceived);
    window.addEventListener("attendanceUpdated", handleAttendanceUpdated);

    return () => {
      websocket.disconnect();
      window.removeEventListener("attendanceScanReceived", handleScanReceived);
  ...
  const filteredAttendance = useMemo(() => {
    return attendanceData.filter((attendance) => {
      const matchesDepartment =
        ...
        attendance.department === filters.department;
      const matchesDesignation =
        filters.designation === "All" ||
        attendance.designation === filters.designation;
      const matchesLevelOfWork =
        filters.levelOfWork === "All" ||
        attendance.levelOfWork === filters.levelOfWork;
  ...
  const calculateOvertime = (checkOut) => {
    ...
    if (
      ...
      checkOutMinutes <= overtimeEndMinutes
    ) {
    ...
    if (isNextDay) {
      ...
      if (
        checkOutMinutes >= nextDayOvertimeStart &&
        checkOutMinutes <= nextDayOvertimeEnd
      ) {
    ...
    if (isNextDay) {
      ...
      if (checkOutMinutes > nextDayExtraOvertimeStart) {
        const extraOvertimeMinutes =
          checkOutMinutes - nextDayExtraOvertimeStart;
        return Math.round((extraOvertimeMinutes / 60) * 10) / 10;
      }
    }
  ...
  const presentEmployees = filteredAttendance.filter(
    (emp) => emp.status === "Present"
  ).length;
  const presentOnTimeEmployees = filteredAttendance.filter(
    (emp) => emp.checkIn && getCheckInStatus(emp.checkIn) === "On Time"
  ).length;
  const presentConsideredEmployees = filteredAttendance.filter(
    (emp) => emp.checkIn && getCheckInStatus(emp.checkIn) === "Considered"
  ).length;
  const presentLateEmployees = filteredAttendance.filter(
    (emp) => emp.checkIn && getCheckInStatus(emp.checkIn) === "Late Login"
  ).length;
  const onLeaveEmployees = filteredAttendance.filter(
    (emp) =>
      emp.status === "Leave-Earn" ||
      emp.status === "Leave-Maternity" ||
      emp.status === "Leave-Casual" ||
      emp.status === "Leave-Sick" ||
      emp.status === "Leave-WithOutPay"
  ).length;
  const leaveEarnEmployees = filteredAttendance.filter(
    (emp) => emp.status === "Leave-Earn"
  ).length;
  const leaveMaternityEmployees = filteredAttendance.filter(
    (emp) => emp.status === "Leave-Maternity"
  ).length;
  const leaveCasualEmployees = filteredAttendance.filter(
    (emp) => emp.status === "Leave-Casual"
  ).length;
  const leaveSickEmployees = filteredAttendance.filter(
    (emp) => emp.status === "Leave-Sick"
  ).length;
  const leaveWithoutPayEmployees = filteredAttendance.filter(
    (emp) => emp.status === "Leave-WithOutPay"
  ).length;
  const absentEmployees = filteredAttendance.filter(
    (emp) => emp.status === "Absent"
  ).length;
  const totalWorkingHours = filteredAttendance.reduce(
    (sum, emp) => sum + calculateWorkingHours(emp.checkIn, emp.checkOut),
    0
  );
  // Only calculate overtime for Workers
  const totalOvertime = filteredAttendance.reduce(
    (sum, emp) =>
      emp.levelOfWork === "Worker"
        ? sum + calculateOvertime(emp.checkOut)
        : sum,
    0
  );
  const totalExtraOvertime = filteredAttendance.reduce(
    (sum, emp) =>
      emp.levelOfWork === "Worker"
        ? sum + calculateExtraOvertime(emp.checkOut)
        : sum,
    0
  );

  // Calculate snacks and night bill eligibility counts
  const snacksEligibleCount = filteredAttendance.filter(
    (emp) =>
      getSnacksEligibility(
        emp.levelOfWork,
        calculateExtraOvertime(emp.checkOut)
      ) === "Eligible"
  ).length;

  const nightBillEligibleCount = filteredAttendance.filter(
    (emp) =>
      getNightBillEligibility(
        emp.levelOfWork,
        calculateExtraOvertime(emp.checkOut)
      ) === "Eligible"
  ).length;
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                handleTimeSettingChange("safeEntryTime", e.target.value)
              }
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                handleTimeSettingChange("lunchStartTime", e.target.value)
              }
              className="w-full h-10 rounded border border-gray-300 px-3 focus:outline-none focus:ring-2 focus:ring-orange-500"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                handleTimeSettingChange("lunchEndTime", e.target.value)
              }
              className="w-full h-10 rounded border border-gray-300 px-3 focus:outline-none focus:ring-2 focus:ring-orange-500"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                handleTimeSettingChange("safeExitTime", e.target.value)
              }
              className="w-full h-10 rounded border border-gray-300 px-3 focus:outline-none focus:ring-2 focus:ring-orange-500"
            />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                handleFilterChange("levelOfWork", e.target.value)
              }
              className="w-full h-10 rounded border border-gray-300 px-3 focus:outline-none focus:ring-2 focus:ring-orange-500">
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Extra Overtime{" "}
                  <span className="text-blue-600">(Workers Only)</span>
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
              ...
              {filteredAttendance.map((attendance) => (
                <tr key={attendance.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <button
                      ...
                      className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full cursor-pointer hover:opacity-80 transition-opacity ${
                        attendance.attendanceCount >= 4
                          ? "bg-green-100 text-green-800"
                          : attendance.attendanceCount >= 2
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
                      className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                        getCheckInStatus(attendance.checkIn) === "On Time"
                          ? "bg-green-100 text-green-800"
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-blue-600">
                    {getRoundedCheckOut(attendance.checkOut)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {formatWorkingHours(
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {attendance.levelOfWork === "Worker" ? (
                      formatOvertime(
                        calculateExtraOvertime(attendance.checkOut)
                      )
                    ) : (
                      <span className="text-gray-400">N/A</span>
                    )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-semibold text-red-600">
                {formatOvertime(totalExtraOvertime)}
              </div>
              <div className="text-sm text-gray-500">
                Worker Extra Overtime Only
              </div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-semibold text-green-600">
            ...
            <div className="text-center">
              <div className="text-2xl font-semibold text-purple-600">
                {nightBillEligibleCount}
              </div>
              <div className="text-sm text-gray-500">Night Bill Eligible</div>
            </div>
          </div>
          <div className="mt-2 text-xs text-gray-500 text-center">
            * Overtime and Extra Overtime calculations are only applicable for
            Workers
          </div>
        </div>
      </div>
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-gray-900">
                        {selectedEmployee.attendanceCount}
                      </p>
                    </div>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
            <h3 className="text-lg font-semibold mb-4">
              Edit Attendance - {editingEmployee.name}
            </h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleEditFormChange("checkIn", e.target.value)
                  }
                  className="w-full h-10 rounded border border-gray-300 px-3 focus:outline-none focus:ring-2 focus:ring-orange-500"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleEditFormChange("checkOut", e.target.value)
                  }
                  className="w-full h-10 rounded border border-gray-300 px-3 focus:outline-none focus:ring-2 focus:ring-orange-500"
                />
              </div>
