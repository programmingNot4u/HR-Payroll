...
const SectionIcon = ({ name, selected = false, highlighted = false }) => {
  const common = `h-4 w-4 transition-colors ${
    selected ? "text-white" : highlighted ? "text-orange-600" : "text-gray-500"
...
export default function AdminSidebar({
  ...
  const filteredSections = sections
    .map((section) => ({
      ...section,
      items: section.items.filter((item) => canAccessPage(item)),
    }))
  ...
  const [openSections, setOpenSections] = useState(() => {
    ...
    if (selectedItem) {
      const owner = filteredSections.find((s) =>
        s.items.includes(selectedItem)
      );
          ...
          return (
            <div key={section.title} className="mb-2">
              <button
                ...
                <span
                  className={`text-sm ${
                    isParentSelected ? "text-orange-600" : "text-gray-500"
                  }`}>
                  {isOpen ? "−" : "+"}
                </span>
              </button>
